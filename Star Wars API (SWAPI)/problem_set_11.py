...
def board_passengers(starship, passengers):
    ...
    max_passengers = utl.convert_to_int(starship.get("max_passengers"))
    starship['passengers_on_board'] = passengers[:max_passengers] if max_passengers is not None else []
    return starship
...
def convert_gravity_value(value):
    ...
    try:
        value_cleaned = value.lower().replace("standard", "").strip()
        return utl.convert_to_float(value_cleaned)
    except:
...
def create_person(data, planets=None):
    ...
    if 'height' in person:
        person['height_cm'] = utl.convert_to_float(person['height'])

    if 'mass' in person:
        person['mass_kg'] = utl.convert_to_float(person['mass'])

    if 'homeworld' in person:
        homeworld = None
        if planets:

            mandalorian_planet = get_mandalorian_data(planets, person['homeworld'])
            if mandalorian_planet:
                homeworld = create_planet(mandalorian_planet)
            else:
                mandalorian_planet = get_swapi_resource(person['homeworld'])
    ...
    new_order = ['url', 'name', 'birth_year', 'height_cm', 'mass_kg', 'homeworld', 'force_sensitive']

    return {key: person.get(key) if key in person else None for key in new_order}
...
def create_starship(data):
    ...
    return {
        ...
        "crew_members": utl.convert_to_none(starship.get("crew_members"), utl.NONE_VALUES),
        "max_passengers": utl.convert_to_int(starship.get("passengers")),
        "passengers_on_board": utl.convert_to_none(starship.get("passengers_on_board"), utl.NONE_VALUES),
...
def create_vehicle(data):
    ...
    return {
        ...
        "crew_members": utl.convert_to_none(vehicle.get("crew_members"), utl.NONE_VALUES),
        "max_passengers": utl.convert_to_int(vehicle.get("passengers")),
        "passengers_on_board": utl.convert_to_none(vehicle.get("passengers_on_board"), utl.NONE_VALUES),
...
def get_mandalorian_data(mandalorian_data, filter):
    ...
    """
    for data_dict in mandalorian_data:
        if data_dict.get('name', '').lower() == filter.lower():
            return data_dict
    return None
...
def update_planets_visited(data, planet):
    ...
    """
    if 'planets_visited' not in data:
...
def main():
    ...
    tatooine = create_planet(get_swapi_resource(SWAPI_PLANETS + "1/"))

    # Problem 3.7 Write to file
    # TODO call function
    utl.write_json("stu-tatooine.json", tatooine)
    print(f"\n3.7 tatooine = {tatooine}")
    ...
    # TODO call function
    sand_crawler = create_vehicle(get_swapi_resource(SWAPI_VEHICLES + "4/"))
    ...
    # TODO call function
    razor_crest_data = get_mandalorian_data(mandalorian_starships, "Razor Crest")
    mandalorian_nevarro = get_mandalorian_data(mandalorian_planets, "Nevarro")
    razor_crest = create_starship(razor_crest_data)
    ...
    # 6.4 Get Greef Karga
    # TODO call function
    greef_karga_data = get_mandalorian_data(mandalorian_people, 'Greef Karga')
    greef_karga = create_person(greef_karga_data, mandalorian_planets)
    ...
    # TODO call function
    cara_dune = create_person(get_mandalorian_data(mandalorian_people, 'Carasynthia Dune'), planets=mandalorian_planets)
