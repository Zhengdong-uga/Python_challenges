...
def convert_published_date_value(data):
    ...
    for article in data:
        # Check if the 'pub_date' key exists and convert its value to datetime
        if "pub_date" in article:
            # Insert a colon in the UTC offset to make it isoformat compatible
            date_str = article["pub_date"]
            if date_str[-5] == "+":
                date_str = date_str[:-2] + ":" + date_str[-2:]
            article["pub_date"] = datetime.fromisoformat(date_str)
    return data
...
def format_author_name(person):
    ...
    last_name = person['lastname'].capitalize()
    first_name = person['firstname'].capitalize()
...
def show_archival_status(data, active_year_threshold):
    ...
    return [
        (article['headline']['main'], "Active" if article['pub_date'].year >= active_year_threshold else "Archived")
        for article in data
    ]
...
def main():
    ...
    for article in nyt_tech:
        # Get organization names from the current article
        org_names = get_organization_names(article)

        # If org_names is not empty
        if org_names:
            # Iterate through org_names and add unique names to tech_organizations
    ...
    unique_authors = []
    # 7.3.1-7.3.6
    for article in nyt_tech_raw:
        authors = get_author_names(article)
        if authors:
            for name in authors:
                if name and name not in unique_authors:
