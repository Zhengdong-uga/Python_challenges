...
def to_int(value):
    ...
    try:
        # Use to_float to handle strings with commas and fractional parts
...
def to_list(value, delimiter=None):
    ...
    try:
        ...
        # Convert the value to a list using the provided delimiter
        if delimiter:
...
def to_none(value, none_values):
    ...
    try:
        # Remove leading/trailing spaces and perform case insensitive comparison
        cleaned_value = value.strip().lower()
        for none_value in none_values:
            if cleaned_value == none_value.lower():
                return None
        return value
    except:
        return value
