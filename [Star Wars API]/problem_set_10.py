...
def get_homeworld(identifier, url):
    ...
    if identifier.startswith("https://"):
        planet_data = get_swapi_resource(identifier)
    else:
        planet_data = get_swapi_resource(url, {"search": identifier})["results"][0]
...
def main():
    ...
    # Fetching data for Leia Organa
    params = {"search": "Leia Organa"}
    swapi_leia = get_swapi_resource(SWAPI_PEOPLE, params)["results"][0]


    r2d2_homeworld = get_homeworld(swapi_r2d2["homeworld"], SWAPI_PEOPLE)
    c3po_homeworld = get_homeworld(swapi_c3po["homeworld"], SWAPI_PEOPLE)
    leia_homeworld = get_homeworld(swapi_leia["homeworld"], SWAPI_PEOPLE)
    ...
    pp.pprint(r2d2_homeworld)
    # pp.pprint(c3po_homeworld)
    # pp.pprint(leia_homeworld)
